...
class HttpServer(object):
    ...
    def http_demo(self,clientSocket):
        ...
        clientSocket.send(bytes(responses.encode('utf-8')))  # 注意要写明编码类型.utf-8 会导致浏览器乱码
        #print(responses)


def main():
    # 使用sys中的argv方法，动态的给应用程序传递 框架中的方法作为参数
    framework_name,app_name = sys.argv[1].split(':')
    #print(framework_name,app_name)
    '''
    framework_name = framework
    app_name = application
    '''
    m = __import__(framework_name)#动态导入模块
    #framework_name.app_name  虽然理解上可以用app_name对应的application类直接调用，但是 spilit返回的是字符串，而不是想元祖那样  一个key对应的values值那样，可以直接使用；
    # 这个相当于调用框架中的app_name方法一样，但实际却不存在这个方法
    #想要直接调用appliaction类，需要用getattr,第一个参数是模块，第二个参数是模块中的应用程序或者是类，返回值就是这个应用程序或者说是类的引用
    # application = getattr(framework_name,app_name)
    # app = application(urls) 原本是需要初始化类的吗，但是因为路由不能传递的原因，所以需要先将框架中的类先初始化对象，直接拿到实例的对象app 来使用
    app = getattr(m,app_name)
    #print(app)
    # 创建对象
    httpSocket = HttpServer(app)
