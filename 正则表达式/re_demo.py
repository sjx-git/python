...
class Re_demo(object):
    ...
    表示字符
        ...
        [ ] 匹配[ ]中列举的字符
            [0-9] 等同于\d；
            [^0-9]等同于\D ^符号在[]中的时候 表示非 的意思，而不是以xxx开始
        \d	匹配数字，即0-9
        \D	匹配非数字，即不是数字
        \s	匹配空白，即 空格，tab键
        \S	匹配非空白
    ...
    匹配分组
        |	匹配左右任意一个表达式
        (ab)	将括号中字符作为一个分组
            \num	引用分组num匹配到的字符串,从1开始
        (?P<name>)	分组起别名，
            记得要将正则表达式也同时括起来（？P<name>\w*）
    ...
    def re_demos(self):
        ...
        #匹配0-100
        # num = re.match(r'[1-9]?\d?$|100','10')#个位数出现就是1-9 不出现就是匹配后边的\d 再用 管道加上100
        # print(num.group())
